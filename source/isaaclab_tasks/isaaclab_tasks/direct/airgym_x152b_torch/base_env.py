...
class AirGymX152bBaseEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        if self._controller is None:
            ...
            root_pos_local = self._root_pos_local().detach().to(dtype=torch.float32).contiguous()
            root_quat_w = quat_unique(self._robot.data.root_quat_w).detach().to(dtype=torch.float32).contiguous()
            root_lin_vel = self._robot.data.root_lin_vel_w.detach().to(dtype=torch.float32).contiguous()
            root_ang_vel = self._robot.data.root_ang_vel_w.detach().to(dtype=torch.float32).contiguous()
            actions_torch = processed_actions.detach().to(dtype=torch.float32).contiguous()
            dt = float(self.step_dt)

            if self.cfg.ctl_mode == "pos":
                self._controller.set_status(root_pos_local, root_quat_w, root_lin_vel, root_ang_vel, dt)
                cmd_thrusts = self._controller.update(actions_torch)
                ...
                cmd_thrusts = self._controller.update(actions_torch, root_ang_vel, dt)
            else:
                raise ValueError(f"Unsupported control mode: {self.cfg.ctl_mode}")

            self._cmd_thrusts = cmd_thrusts.to(device=self.device, dtype=torch.float32)
    ...
    def _camera_depth_image(self) -> torch.Tensor:
        ...
        if self.cfg.camera_additive_noise_std > 0.0:
            depth = depth + self.cfg.camera_additive_noise_std * torch.randn_like(depth)
        if self.cfg.camera_multiplicative_noise_std > 0.0:
            depth = depth * (
                1.0 + self.cfg.camera_multiplicative_noise_std * torch.randn_like(depth)
            )
        depth = depth.clamp_(0.0, 1.0)
        self._camera_image = depth
        return depth
